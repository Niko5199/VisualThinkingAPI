...
describe("Tests only for Parthers", () => {
    const visualParthers = JSON.parse(fs.readFileSync("visualparthers.json"));
    test("Test for method readJSONVisualParthers", () => {
        expect(visualParthers).toStrictEqual([
            {
    ...
    test("Test for method getVisualParthersEmails", () => {
        const visualParthersEmails = visualParthers.filter((parther) => {
            if (parther.haveCertification === true) return parther.email;
        });
        expect(visualParthersEmails).toStrictEqual([
            {
                id: "6264d5d89f1df827eb84bb23",
                name: "Warren",
                email: "Todd@visualpartnership.xyz",
